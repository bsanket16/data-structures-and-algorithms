...
public class OneSubsetWithSumK {
    ...
    static boolean subset(int[] nums, int i, int n, List<Integer> temp, int sum, int currSum) {
        if (i == n) {
            if (sum == currSum) {
        ...
        if (subset(nums, i + 1, n, temp, sum, currSum)) return true;
